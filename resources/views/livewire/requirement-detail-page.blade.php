            ...
            <div class="p-4 lg:p-6 bg-white border-b border-gray-200 grid grid-cols-2">
                ...
                <div class="col-span-1 text-right">
                    @if($requirement->warehouse_id)
                        @if($requirement->warehouse->team_id==Auth::user()->currentTeam->id)
                            @if(!$imece)
                                <x-danger-button wire:click="$toggle('delete_modal')"> {{ __('Delete Requirement') }} </x-danger-button>
                            @endif
                        @else
                            @if(!$imece)
                                <x-button wire:click="$toggle('satisfy_modal')"> {{ __('Satisfy Requirement') }} </x-button>
                            @endif
                        @endif
                    @endif
                </div>
                ...
                <div class="bg-gray-200 bg-opacity-25 p-6 lg:p-8 flex items-center justify-center">
                    <div class="w-full">
                        <div class="grid grid-cols-1 md:grid-cols-2">
                            ...
                            <div>
                                <div wire:ignore id="mapContainerSatisfier" class="w-full" style="height: 50vh"></div>
                            </div>
                        </div>
                    </div>
                </div>
